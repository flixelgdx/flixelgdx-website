/**
 * Vendored StartupHelper for LWJGL3 desktop launches (damios, Apache-2.0).
 * The project generator injects the correct {@code package} line for each download.
 */
export function buildStartupHelperJava(lwjgl3Package: string): string {
  return STARTUP_JAVA_SOURCE.replace(/<<PACKAGE>>/g, lwjgl3Package);
}

const STARTUP_JAVA_SOURCE = `/*

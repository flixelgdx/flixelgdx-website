...
const JDK_TIPS: Record<JdkVendor, JdkInstallTips> = {
  semeru: {
    label: 'Eclipse OpenJ9 (IBM Semeru) - recommended for FlixelGDX',
    url: 'https://developer.ibm.com/languages/java/semeru-runtimes/downloads/',
    steps: [
      'Pick the "Open Edition" JDK build that matches your OS.',
      'Windows : run the .msi, tick "Set JAVA_HOME" and "Add to PATH".',
      'macOS   : brew install --cask semeru-jdk-open',
      'Ubuntu  : add the IBM Semeru apt repo, then `apt install ibm-semeru-open-17-jdk`.',
    ],
  },
  graalvm: {
...
const IDE_TIPS: Record<IDE, IdeTips | null> = {
  idea: {
    ...
    steps: [
      ...
      '3. Use the pre-made "Run Desktop" run configuration at the top-right toolbar.',
    ],
  },
  eclipse: {
    ...
    steps: [
      ...
      '3. Right-click the `lwjgl3` subproject -> Run As -> "Gradle Tasks: :run".',
    ],
  },
  vscode: {
    ...
    steps: [
      ...
      '3. Press F5 (a "Run Desktop" launch is already in .vscode/launch.json).',
    ],
  },
  none: null,
};
...
function readme(o: GeneratorOptions): string {
  // Always emitted as a plain Markdown README.md too — but the README.txt
  // generated alongside (see buildProjectFiles) is the version the project
  // generator markets in its "open this and read the instructions" flow.
...
function readmeTxt(o: GeneratorOptions): string {
  // Plain-text version (no Markdown formatting) for users who unzip and
  // open the README.txt with Notepad / TextEdit / cat. Same content,
  // stripped of Markdown headers and code-block fences.
  return readme(o)
    .replace(/^#+\s+/gm, '')
    .replace(/\*\*/g, '')
    .replace(/`/g, '');

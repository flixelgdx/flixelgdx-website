...
title: Core Concepts
sidebar_position: 3
description: The 7 essential building blocks every FlixelGDX game is built on.
---
...
This page walks through the 7 essential classes and systems that form the foundation of every FlixelGDX game.
Each section includes practical code examples in both Java and Kotlin.
...
| `Flixel.touches` | Touchscreen input |
...
:::warning[World size stays the same regardless of window size]
...
Although it might not seem like it at first, this piece of info is quite important to know, as there are certain
stages during the game's launch sequence where you can — and can't — do certain things. To keep it simple, the
different stages have been broken down into several steps.

### A. Triggering the Launcher

This is where the dedicated platform launcher is executed for the first time, typically in the main method:

<Tabs groupId="lang">
<TabItem value="java" label="Java" default>

```java
// For example, in the desktop/LWJGL3 launcher.
public final class MyGameLwjgl3Launcher {
...
At this stage, it's extremely bare bones — basically nothing is available yet and multiple core systems attempted
to be used in this phase will crash. This includes OpenGL context (or just graphics in general), libGDX `FileHandle`s,
asset handling, and so on.

### B. Concrete Implementation Bootstrapping

Once a platform launcher is executed, it bootstraps its own different subservices that specific platform uses. For
example, the alert system via `FlixelAlerter` on desktop works very differently than it does on browser, Android,
and so on; in accordance to that, it assigns its own implementation that fulfills that contract. This also includes
setting the runtime mode based on the arguments passed in the launcher and configuring certain systems (like the
debug manager) accordingly.

### C. Initializing FlixelGDX via `Flixel.initialize(FlixelGame)`

Once the platform supplies its own system-specific implementations, `Flixel.initialize(FlixelGame)` is then executed.
Inside of this method, all of the core systems such as `keys`, `mouse`, `assets`, `tween`, `sound`, etc. are assigned.

### D. Application Object Created

At this stage, the provided `FlixelGame` class is then passed on to the platform's `Application` object, and libGDX takes
over from here. At this phase, libGDX creates the OpenGL/graphics context, its own sub systems, the window, etc.

In the framework's source code, steps B, C and D roughly look like this:

```java
// Here, it assigns its own implementations for cross-platform interfaces provided by flixelgdx-core.
Flixel.alert = new FlixelLwjgl3Alerter();
Flixel.window = new FlixelLwjgl3Window();
// ... and so on.

// Runtime mode is also set up here. Below you can also see the platform-specific debugger for desktop is provided.
...
// FlixelGDX's systems get configured and set up. Things like the input managers, sound system and various
// others are all handled and bootstrapped here.
Flixel.initialize(game);

// Finally, the application object is created and libGDX takes the wheel from here. It creates the graphics context,
// its own managers, the window, and much more.
new Lwjgl3Application(game, configuration);
```

### E. `FlixelGame.create()` is Executed

This is the last phase. Since graphics and libGDX's own systems are finally available, components like `FlixelSpriteBatch`,
the camera system and libGDX's `InputProcessor` (which FlixelGDX's input systems hook into) can be utilized.

After all systems have been configured, the provided initial state is then switched to and the game loop begins.

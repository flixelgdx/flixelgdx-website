...
// Play in reverse (goal to starting value).
FlixelTween.tween(sprite, new FlixelTweenSettings()
...
```java
// Flash the sprite from white back to its original color over 0.2 seconds.
FlixelTween.color(sprite, FlixelColor.WHITE, FlixelColor.WHITE.withAlpha(0),
  new FlixelTweenSettings().setDuration(0.2f));

// Cycle the sky background between dawn and day colors.
FlixelTween.color(sky, FlixelColor.fromRGB(255, 180, 100), FlixelColor.fromRGB(135, 206, 235),
  new FlixelTweenSettings()
    .setType(FlixelTweenType.PINGPONG)
    .setDuration(5.0f));
```
...
FlixelTween.angle(sprite, 0f, 360f,
  new FlixelTweenSettings()
    .setType(FlixelTweenType.LOOPING)
    .setDuration(2.0f));
```
